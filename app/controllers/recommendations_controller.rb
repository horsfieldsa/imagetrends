class RecommendationsController < ApplicationController
  ...
  # GET /recommendations
  def index
    @recommended = get_user_recommendations(current_user.id)
  end
  ...
  def get_useritem_recommendations(user_id, item_id)

    collection = []
    recommendations = []

    uri = URI("#{ENV["RECOMMEND_API"]}?user_id=#{user_id}&item_id=#{item_id}")
    recommendations_logger.info("Recommendation API: #{uri}")
